            ...
            angular.extend(this, {
                $onInit: function $onInit() {
                    ...
                    socket.on('playValue', function (socket) {
                        socket.tic.forEach(function (row) {
                            // console.log(socket.tic, row)
                            console.log(socket.tic[socket.ptidx][socket.idx].value);
                            return row[socket.idx].player == _this.currentUser.name ? socket.tic[socket.ptidx][socket.idx].value = "x" : socket.tic[socket.ptidx][socket.idx].value = "o";
                        });
                        _this.tic = socket.tic;
                    });
                    //emit on click
                    this.click = function (ptidx, idx) {
                        var tic = _this.tic;
                        tic[ptidx][idx].player = _this.currentUser.name;
                        socket.emit('play', {
                            ptidx: ptidx,
                            idx: idx,
                            tic: tic
